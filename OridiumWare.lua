...
local MovementTab = Window:CreateTab({ name = "Movement",       icon = 0 })
local LightingTab = Window:CreateTab({ name = "Lighting & Map", icon = 0 })
local PshadeTab   = Window:CreateTab({ name = "Pshades Shader", icon = 0 })
...
MainTab:CreateButton({
   name = "Death Aura",
   description = "Toggle death aura particles (R6)",
   callback = function()
...
MainTab:CreateButton({
   name = "Korblox",
   description = "Apply Korblox right leg (R6)",
   callback = function()
...
MovementTab:CreateButton({
   name = "Auto Edge Trimp",
   description = "130 stud launch on world part edges",
   callback = function()
      if edgeActive then
         edgeActive=false
...
MovementTab:CreateButton({
   name = "Auto Bounce",
   description = "130 stud bounce on slopes",
   callback = function()
...
local walkActive=false local walkConn=nil
local walkStuck=false local walkNormal=Vector3.zero

MovementTab:CreateButton({
   name = "Oridium Walk",
   description = "Hold W + Space on a wall to climb",
   callback = function()
      if walkActive then
...
LightingTab:CreateButton({
   name = "Open Map Loader",
   description = "Browse and load maps",
   callback = function()
...
PshadeTab:CreateButton({
   name = "Load PShade Ultimate",
   description = "Loads external shader UI (once per session)",
   callback = function()
